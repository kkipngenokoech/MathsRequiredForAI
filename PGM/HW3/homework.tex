...
\begin{enumerate}
   ...
   so, our joint likelihood is:
    \[
    p(y, \theta, \eta) = \prod_{g=0}^{379} \left[\frac{e^{-\theta_{g1}}\theta_{g1}^{y_{g1}}}{y_{g1}!} \cdot \frac{e^{-\theta_{g2}}\theta_{g2}^{y_{g2}}}{y_{g2}!}\right] \times \mathcal{N}(\text{home}; 0, \tau_0^{-1}) \prod_{t=1}^{19} \mathcal{N}(\text{att}_t; \mu_{\text{att}}, \tau_{\text{att}}^{-1}) \prod_{t=1}^{19} \mathcal{N}(\text{def}_t; \mu_{\text{def}}, \tau_{\text{def}}^{-1})\]
